...
class Application {
    constructor() {
        ...
        // Remove global DDoS protection (rate limiting)
        ...
        // Add cookie parser BEFORE csurf if using cookie option
                ...
                const ollamaResponse = await fetch('http://localhost:11434/api/chat', {
                    ...
                    body: JSON.stringify({
                        ...
                        messages: [
                            {
                                role: 'system',
                                content: `Analyze these ${logs.length} security alerts. The alerts are from Suricata Intrusion Detection System.
                                Provide:
    ...
    this.app.get('/api/alerts/export-json', async (req, res) => {
        try {
            const command = `bash -c "grep '\\"event_type\\":\\"alert\\"' /var/log/suricata/eve.json"`;
